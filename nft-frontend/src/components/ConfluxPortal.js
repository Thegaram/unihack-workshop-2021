...
import { Drip, Conflux } from 'js-conflux-sdk'
...
export default class ConfluxPortal extends PureComponent {
  ...
  connectConfluxPortal = async () => {
    ...
    const conflux = new Conflux({
      networkId: Number(window.conflux.networkVersion),
    });
    conflux.provider = window.conflux;
    this.props.onConnected(conflux);

    this.refreshBalance(conflux)
  }

  refreshBalance = async (conflux) => {
    if (!this.state.account) {
      return
    }
    this.setState({ balance: '' })
    const balance = await conflux.getBalance(this.state.account)
    this.setState({ balance: (new Drip(balance)).toCFX() })
  }
        ...
        <div className="mt-3">
          ...
          <div className="d-flex align-items-center">
            ...
            <span
              ...
              onClick={() => this.refreshBalance(this.props.conflux)}
            >refresh</span>
          </div>
        </div>
      )
